...
async function sendToMake(scenario: string, data: Record<string, unknown>) {
...
async function sendNotificationEmail(booking: {
  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Good Hands <bookings@beautysalonlisbon.com>',
        to: [notifyEmail],
...
const VALID_SERVICES = ['hair', 'nails', 'skincare', 'makeup', 'wellness', 'eyebrows', 'education', 'wedding', 'retreat', 'corporate']
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validatePhone(phone)) {
      return NextResponse.json({ error: 'Invalid phone number. Please provide a valid number with at least 8 digits.' }, { status: 400 })
    }

    if (!validateDate(date)) {
      return NextResponse.json({ error: 'Please select a date today or in the future' }, { status: 400 })
    }
